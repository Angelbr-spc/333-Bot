...
const handler = async (m, { conn, args }) => {
  ...
  const apiUrl = `https://api.nekorinn.my.id/ai/chatbot?ai_name=Alejandro&text=${encodeURIComponent(text)}`;
  ...
  try {
    ...
    if (json && json.data) {
      await conn.sendMessage(m.chat, { text: `🤖 *Alejandro dice:* ${json.data}` }, { quoted: m });
    } else {
